<template>
    <div>
        ...
        <div class="home-banner" :style="cover">
            <div class="banner-container">
                ...
                <div class="blog-contact">
                    <a v-if="isShowSocial('qq')" class="mr-5 iconfont iconqq" target="_blank" :href="
                        'http://wpa.qq.com/msgrd?v=3&uin=' +
                        blogInfo.websiteConfig.qq +
                        '&site=qq&menu=yes'
                    " />
                    <a v-if="isShowSocial('github')" target="_blank" :href="blogInfo.websiteConfig.github"
                        class="mr-5 iconfont icongithub" />
                    <a v-if="isShowSocial('gitee')" target="_blank" :href="blogInfo.websiteConfig.gitee"
        ...
        <v-row class="home-container">
            <v-col md="9" cols="12">
                <v-card class="animated zoomIn article-card" style="border-radius: 12px 8px 8px 12px"
                    v-for="(item, index) of state.articleList" :key="item.id">
                    <!-- 文章封面图 -->
                    <div :class="isRight(index)">
                        <router-link :to="'/articles/' + item.id">
                            <v-img class="on-hover" width="100%" height="100%" :src="item.articleCover" id="conta"/>
                        </router-link>
                    </div>
                    <!-- 文章信息 -->
                    <div class="article-wrapper">
                        ...
                        <div class="article-info">
                            ...
                            <router-link style="display:inline-block" :to="'/tags/' + tag.id" class="mr-1"
                                v-for="tag of item.tagDTOList" :key="tag.id">
                    ...
                    <v-card class="blog-card animated zoomIn mt-5">
                        ...
                        <div class="web-info">
                            <div style="padding:4px 0 0">
                                运行时间:<span class="float-right">{{ state.time }}</span>
                            </div>
                            <div style="padding:4px 0 0">
        ...
        <!-- 提示消息 -->
        <v-snackbar v-model="state.tip" top color="#49b1f5" :timeout="2000">
            按CTRL+D 键将本页加入书签
        </v-snackbar>
    </div>
</template>
...
import TypeWriter from '@/components/component/TypeWriter.vue';
...
const cover = computed(() => {
    const coverurl = "https://www.static.ysongblog.cn/articles/cb07f2a2e340d7a5679296fb51ce499f.jpg";
    return {
...
const runtime = () => {
    var timeold =
        new Date().getTime() -
        new Date(blogInfo.value.websiteConfig.websiteCreateTime).getTime();
